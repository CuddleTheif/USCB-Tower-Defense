	...
	public UpgradeMenu(Tower tower){
		...
		/* Get the prices of the possible upgrades of this tower */
			...
			for(int i=0;i<upgrades.size();i++){
				...
				/* Get the image of the current attribute */
					if(i<2) prices[i] = (int)tower.getAttribute(upgrades.get(i))*2;
					else prices[i] = (int)spawn.getAttribute(upgrades.get(i))*2;
									...
									/* Check to see if the tower, its entity, or both have the attribute */
										...
										if(spawn.hasAttribute(upgrades[selected])){
											...
											/* increase the spawn's attribute */
												spawn.setAttribute(upgrades[selected], (int)spawn.getAttribute(upgrades[selected])+increase);
												
												
											/* Update the price of the upgrade */
												if(upgrades[selected]==Attribute.MAX_COOLDOWN) prices[selected] =  1000 - (int)spawn.getAttribute(upgrades[selected])*2;
												else prices[selected] = (int)spawn.getAttribute(upgrades[selected])*2;
										
										}// End if(spawn.hasAttribute(upgrades[selected]))
										if(tower.hasAttribute(upgrades[selected])){
											
											/* increase the tower's attribute */
												tower.setAttribute(upgrades[selected], (int)tower.getAttribute(upgrades[selected])+increase);
										
												
											/* Update the price of the upgrade */
												if(upgrades[selected]==Attribute.MAX_COOLDOWN) prices[selected] = 1000-(int) tower.getAttribute(upgrades[selected])*2;
												else prices[selected] = (int) tower.getAttribute(upgrades[selected])*2;

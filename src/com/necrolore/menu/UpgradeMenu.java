...
import com.necrolore.util.Pair;
					...
					if(selected>=0){
						...
						if(((Level)getWorld()).getGold()>=prices[selected]){
							...
							/* Reset the price of the upgrade and increase the price of the tower */
								upgrades[0].getA().setAttribute(Attribute.PRICE, (int)((Integer)upgrades[0].getA().getAttribute(Attribute.PRICE)-prices[selected]*Attribute.PRICE.getPriceMuti()));
